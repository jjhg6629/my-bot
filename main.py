...
TOKEN = "8784816871:AAFVG4FT-F0qq6OxrteVGTFyErQT0BZvzNo" 

# ── Join requirements ─────────────────────────────────────────────────────────
join_channel_id = -1001506965666       # Replace with your channel ID
join_chat_id    = -1004499245974       # Replace with your group ID

CHANNEL_LINK = "https://t.me/+imAsaWIJ91cwYjQx"
GROUP_LINK   = "https://t.me/ggdggdhg"
...
#     await message.reply(
#         f"{pe(E['cross'])}{bold('No Proxy Set!')}\n\n"
#         f"{pe(E['warn'])}{bold('You must add a proxy')}\n"
#         f"{pe(E['next'])}{bold('Use:')} /proxy host:port"
#     )
#     return
...
loading_msg = await message.reply(
    ...
    try:
        setup_result = await ayden.process_payment()
    except Exception as e:
    ...
    if _is_checkout_expired(setup_result):
        ...
        await safe_edit(loading_msg,
            ...
            f"{pe(E['warn'])} {bold('This Adyen link is not valid or expired.')}"
        )
        return
...
async def _usd1_single_cmd(message: types.Message, gate_label: str, checker):
    ...
    proxy_list = get_user_proxies(user_id) or [None]
    bin_num = cc_str.split("|")[0][:6]
    loading_msg = await message.reply(
        f"{pe(E['loading'])} {bold('Checking CC...')}\n\n"
        f"{pe(E['bolt'])} {bold('CC:')} <tg-spoiler>{cc_str}</tg-spoiler>\n"
